// src/features/services/ui/templates/LandingTemplate.tsx
...
export default function LandingTemplate({ data, locale }: ServiceDetailViewProps) {
  ...
  const serviceKey = (data.key ?? "landing") as ServiceKey;

  return (
    <>
      {/* HERO — sin separación vertical */}
      <Section className="!py-0">
        <Container size="xl">
          ...
          {/* Subtítulo — más aire debajo del título */}
      ...
      {/* FEATURES — más espacio respecto al bloque anterior */}
      {(hasColA || hasColB) && (
        <Section className="!py-0">
          <Container size="xl" className="mt-[var(--radius-2xl)]">
            ...
            <div className="mt-[calc(var(--radius-xl)*1.25)] flex items-center justify-between gap-[var(--radius-md)]">
              {/* Tags: solo texto con #, sin chip/borde/fondo */}
              {hasTags ? (
              ...
              {/* Botón que abre el modal (misma fila, derecha) */}
