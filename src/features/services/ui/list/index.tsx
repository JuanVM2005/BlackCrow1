...
export default function ServicesList({ locale, items, className }: ServicesListProps) {
  ...
  return (
    <Grid
      role="list"
      className={["grid-cols-1 md:grid-cols-2 gap-(--space-6)", className].filter(Boolean).join(" ")}
    >
      {items.map((item) => {
        ...
        return (
          <article
            role="listitem"
            key={item.key}
            className="rounded-lg border border-(--border) bg-[var(--surface-raised, var(--surface-base))]"
          >
